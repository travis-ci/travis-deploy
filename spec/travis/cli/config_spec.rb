...
describe Travis::Trvs::Config do
  let(:shell)  { Mock::Shell.new }
  let(:config) { "staging:\n  foo: bar" }

  before :each do
    Travis::Trvs::Config.any_instance.stub(:clean? => true)
    Travis::Trvs::Config.any_instance.stub(:run)
  ...
  describe 'sync' do
    it 'fetches the config from the keychain' do
      command = Travis::Trvs::Config.new(shell, 'staging', {})
